...
Each others interval $j>0$ are initialized to $status_j = \textsc{ToCompute}$ and their lower bounds are set to the minimal element that is  $s_j = \bot$. 

This algorithm works in rounds. Every rounds, each interval $j$ set to $\textsc{ToCompute}$ or $\textsc{Validated}$ is calculated and the processor calculates new bound for the interval $j+1$. An interval set to $\textsc{ToCompute}$ is then set to $\textsc{Computed}$, and an interval to $\textsc{Validated}$ becomes $\textsc{Done}$ after the computation.
 Let us call $s_j^{-1}$ the lower bound calculated for the interval $j$ at the previous round. At the end of each rounds, the intervals $j$ with new calculated bounds are set to $\textsc{ToCompute}$ if $s_j^{-1} < s_j$. In the case where $s_j^{-1} = s_j$, then $status_j$ is set to $\textsc{Coupled}$. 
 
Afterward, a new step spreads the information about the correct trajectory starting to the interval $0$. If an interval $j$ is $\textsc{Validated}$ or $\textsc{Done}$, the interval $j+1$ is set to $ \textsc{Validated}$ if $state_{j+1} = \textsc{Validated}$. This means that the correct trajectory for the interval $j+1$ will be calculated at the next round.
...
We now want to measure the performance of the \textsc{Speculative Sandwich} algorithm with both \textsc{Ordered} and \textsc{Balanced} policies, compared to the One bound algorithm and the sequential computation of the simulations. In the following experiments we thus take a random process with $20$ queues, which ensures us to have a large computing time to make it dominate the network time in the results. In order to choose the size $n$ of the simulation, we made an upstream experiment to determine the average time between the coupling of the two bounds (cf Fig.~\ref{fig:distribs}).  In this part, the experiments are made on $7$ slaves.


\paragraph {Short simulation}
We first tried to look at the performances of the algorithms on a simulation short enough to be sure that there will be no coupling between the two bounds before the end of the simulation, i.e., the simulation is computed sequentially for the beginning of the simulation. We thus choose $n = 100,000$ and we measured the total time of the simulation with a different number of intervals. Fig~\ref{fig:short_time} shows the average total time of $10$ simulations for the three different parallels algorithms and the sequential simulation made on one slave. The measures for the balanced policy starts at $14$ because this policy is the same as the ordered one when the number of intervals is lower than twice the number of processors.
\begin{figure}[]
\centering
...
As we can see, the time of the parallel simulation of the \textsc{Speculative Sandwich} algorithm does not depends of the number of intervals. This make sense since the trajectory is computed "sequentially" because there is no coupling. Hence, one processor will fully compute the correct trajectory communicating at each end of interval with the master. Nevertheless, this time is still longer than the time to compute the full trajectory on a single processor, because there is the network time to take into consideration. In the other hand, since the One bound algorithm works in round, during which each intervals might be updated. The total time of simulation seems to scale linearly when the number of intervals is greater than the number of slaves. Indeed, if there is enough slaves to treat all the intervals in parallel, even if only one processor is computing the correct trajectory, there is no loss of times. Nevertheless, if there is too much intervals, since the round have to be fully computed to go to the next round, the slaves will be used to compute some "useless" intervals and thus we this time is purely lost.
...
First of all, we remark that the \textsc{Speculative Sandwich} algorithm with \textsc{Balanced} policy is always better than the other algorithms.
  Those experiments shows that this gives us some better computation times than a sequential computation. Furthermore, it is interesting to choose correctly the number of intervals to cut the simulations. Indeed, one can observe different phenomenas on the behavior of the curves.
First, we remark that the time of computing is decreasing for up to $8$ intervals. That correspond to the number of slaves plus one. Indeed, the more slaves we have, the faster the algorithm goes. The "plus one" is due to the structure of the algorithm. Indeed, the last interval of the simulation is computed only when the penultimate computes some coupled bounds. Here, we have some intervals greater than the coupling time, and thus each interval is computed two times in parallel, except the first and the last one, that are computed only once. If we add another interval, the algorithm have to wait for a free slave to compute it. One can imagine that each machine will compute two intervals except one that will compute two and add additional compute time to the parallel simulation.
...
Thus, to ensure a minimal computation time of the simulation, one must choose the number of interval considering the coupling time. The most efficient algorithm is the  \textsc{Speculative Sandwich} with \textsc{Balanced} policy, and we can decrease the computation time to it's lower by choosing a number of intervals equal to $k \times l +1$, with $l$ the number of slaves and $k$ chosen such that the size of the intervals $n/k$ is as short as possible, but still greater than the maximal coupling time of the two bounds. 
 ...
 One must try to measure the impact of the number of slaves on the time of the simulation.
 We fixed the number of intervals to $14$ for a simulation of $10$M of events and we measured the total time of the  \textsc{Speculative Sandwich} algorithm with  \textsc{Ordered} policy varying the number of slaves from $1$ to $7$. Fig.~\ref{fig:nbservs} shows the average total time on $100$ simulations for each number of slaves.
\begin{figure}[H]
\centering
\label{fig:nbservs}
...
\end{figure}
As we can see, the more slaves there is, the faster the algorithm runs. The total time on the parallel simulation with one processor is higher to the sequential time, because it is increased by the network communication and the algorithm on the master.
%
