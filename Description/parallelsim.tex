...
Finally, when a processor simulate the interval $j$ and at the beginning $s_j^{min} = s_j^{max}$, the result of the simulation is stored as a part of the trajectory and it sets $status_j = \textsc{Done}$.
...
\end{proposition}
\begin{proof}
We set $t = n/p$ in \textsc{Speculative Sandwich} (the proof works as long as $t <  n/p$). 
...
In this part, we experiment the proposed algorithms on a multicore machine and in a distributed computing setting. The C code used for the experimentations can be found on one of the author's webpage~\cite{webpage}.
